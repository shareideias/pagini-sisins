...
import br.com.associacaoshare.controller.security.ShareAccessManager.Roles.PARTICIPANTE
import br.com.associacaoshare.model.Participante
import br.com.associacaoshare.model.dao.DataAccessObject
import br.com.associacaoshare.model.exception.FalhaNaInscricao
...
class AlunosController(override val kodein: Kodein) : EndpointGroup, KodeinAware {
    ...
    override fun addEndpoints() {
        ...
        get("redacao_entrada1", ::redacao_entrada1, roles(PARTICIPANTE))
        ...
        get("redacao_entrada2", ::redacao_entrada2, roles(PARTICIPANTE))
    ...
    private fun redacao_entrada1(ctx: Context) {
        val participante = ctx.sessionAttribute<Int?>("ID")?.let { dao.getParticipante(it) }
        val errormsg = ctx.cookie("errorMsg")?.let { decode(it, utf8) }
        if (errormsg != null)
            ctx.cookie("errorMsg", "", 0)
        ...
        if (participante != null && curso != null) {
            Redacao1View(errormsg, participante, curso, interruptor).render(ctx)
        } else {
            throw FalhaSessaoException()
        }
    }
    ...
    private fun cadastraCurso1(ctx: Context) {
        ...
        val categoria = ctx.formParam("categoria")?.toInt()
        val redacao1 = ctx.formParam("redacao1")


        if (participante != null && redacao1 != null) {
            dao.updateCurso1inParticipante(participante, id.toInt(), redacao1)
            preselecao1(participante)
            if (categoria == 1) {
    ...
    private fun redacao_entrada2(ctx: Context) {
        val participante = ctx.sessionAttribute<Int?>("ID")?.let { dao.getParticipante(it) }
        val errormsg = ctx.cookie("errorMsg")?.let { decode(it, utf8) }
        if (errormsg != null)
            ctx.cookie("errorMsg", "", 0)
        ...
        val interruptor = dao.getInterruptor()
        if (participante != null && curso != null) {
            Redacao2View(errormsg, participante, curso, interruptor).render(ctx)
        } else {
            throw FalhaSessaoException()
        }
    }
    ...
    private fun cadastraCurso2(ctx: Context) {
        ...
        val id = ctx.formParam("id") ?: ""
        val categoria = ctx.formParam("categoria")?.toInt()
        val redacao2 = ctx.formParam("redacao2")

        if (participante != null && redacao2 != null) {
            dao.updateCurso2inParticipante(participante, id.toInt(), redacao2)
            preselecao2(participante)
            if (categoria == 1) {
